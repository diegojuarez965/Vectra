...
export default function Scanner() {
  ...
  const [cameraPermission, setCameraPermission] = useState<boolean | null>(null); // Permiso cámara 
  ...
  useEffect(() => {
    ...
    const predictWebcam = () => { // Función de predicción en cada frame
      if (
        ...
        if (canvas) {
          ...
          const results: PoseLandmarkerResult = poseLandmarkerRef.current.detectForVideo(
            video,
            startTimeMs
          ); // Realiza la detección de humanos en el frame actual
    ...
    return () => { if (requestRef.current) cancelAnimationFrame(requestRef.current); }; // Limpieza al desmontar
    ...
    <div className="relative w-full max-w-4xl mx-auto aspect-video bg-black rounded-xl overflow-hidden shadow-2xl border border-white/10">
      ...
      {!isModelLoaded && (
        <div className="absolute inset-0 z-50 flex flex-col items-center justify-center bg-background text-white">
          ...
          <p className="text-lg font-medium animate-pulse">Cargando Motores Neuronales...</p>
        </div>
      )}
      ...
      {isModelLoaded && cameraPermission === false && (
        <div className="absolute inset-0 z-40 flex flex-col items-center justify-center bg-black/90 text-white p-6 text-center">
          ...
          <p className="text-white/60 mb-6 max-w-md">
            Vectra necesita verte para analizar tu postura. Por favor, permite el acceso a la cámara en tu navegador y recarga.
          </p>
          <button
